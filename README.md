...
> 表示は解像度 **1920×1080** のTFT画面に合わせています。
...
2. **Tesseract-OCR** をインストール（日本語データ `jpn` も入れておくと安心）
3. このアプリの `worker` フォルダで、必要パッケージをインストール：
   ```bash
   cd worker
...
### ② Overwolfにアプリを読み込む
1. **Overwolf** をインストールして**ログイン**（ログインしないと後の手順で「Unauthorized App」になります）
2. Overwolfの設定で**開発者モード**を有効化
3. 「**Load unpacked extension**」で、このフォルダ（`manifest.json` がある場所）を選択
